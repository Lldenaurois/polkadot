...
///
/// The program should be able to handle `<program-path> prevalidate-worker <socket-path>` invocation.
pub async fn spawn(
	program_path: &Path,
...
pub enum Outcome {
	...
	/// another job.
	Ok {
		result_descriptor: PreValidationResult,
		duration_ms: u64,
...
async fn recv_request(stream: &mut UnixStream) -> io::Result<(PathBuf, Vec<u8>)> {
	...
	let artifact_path = bytes_to_path(&artifact_path).ok_or_else(|| {
		io::Error::new(
			io::ErrorKind::Other,
			"prevalidate pvf recv_request: non utf-8 artifact path".to_string(),
		)
	})?;
...
async fn recv_response(stream: &mut UnixStream) -> io::Result<Response> {
	...
	Response::decode(&mut &response_bytes[..]).map_err(|e| {
		io::Error::new(
			io::ErrorKind::Other,
			format!("prevalidate pvf recv_response: decode error: {:?}", e),
		)
	})
}
...
/// The entrypoint that the spawned prevalidate worker should start with. The socket_path specifies
/// the path to the socket used to communicate with the host.
pub fn worker_entrypoint(socket_path: &str) {
	worker_event_loop("prevalidate", socket_path, |mut stream| async move {
		...
		loop {
			...
			let response = prevalidate_using_artifact(&artifact_path, &params, &executor).await;
			send_response(&mut stream, response).await?;
		}
	});
}

async fn prevalidate_using_artifact(
	artifact_path: &Path,
	params: &[u8],
	...
	let descriptor_bytes =
		match unsafe {
			...
			crate::executor_intf::prevalidate(compiled_artifact, params, spawner.clone())
		 } {
			Err(err) => {
				return Response::format_invalid("prevalidate", &err.to_string());
			}
			Ok(d) => d,
		};
	...
	let result_descriptor = match PreValidationResult::decode(&mut &descriptor_bytes[..]) {
		Err(err) => {
			return Response::InvalidCandidate(format!(
				"prevalidation result decoding failed: {}",
				err
			))
		}
		Ok(r) => r,
