...
async fn import_block(
	...
	assert_matches!(
		...
		AllMessages::ChainApi(ChainApiMessage::BlockHeader(head, h_tx)) => {
			let (hash, header) = hashes[(session-1) as usize].clone();
			assert_eq!(head, hash);
	...
	for i in session.saturating_sub(APPROVAL_SESSIONS)..session {
		assert_matches!(
			overseer_recv(overseer).await,
			AllMessages::RuntimeApi(
				RuntimeApiMessage::Request(
				...
				si_tx.send(
					Ok(
						Some(state.session_info(session.into()).clone().unwrap().clone())
					)
				).unwrap();
			}
		);
...
fn import_block_act_on_block() {
	...
	let state = single_session_state(session, session_info);
	...
	test_harness(|test_harness| async move {
		...
		import_block(&mut virtual_overseer, &state, hashes.as_ref(), head, session).await;
